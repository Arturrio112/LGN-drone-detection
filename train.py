...
CONFIG = {
    "COMMON": {
        ...
        "LR_PATIENCE": 2,
        "COLLAPSE_PATIENCE": 3,
        "SAVE_DIR": f"./models/saved_models_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        "NUM_WORKERS": 0,
...
# ==========================================
# Input: (B, 1, 1, 4000)
# Layer 1: rf=(1,64), stride=(1,32), pad=(0,16) -> Output Width: 125
# Pooling 1: kernel=(1,4), stride=(1,4)           -> Output Width: 31
# Layer 2: rf=(1,16), stride=(1,4), pad=(0,8)    -> Output Width: 8
# Pooling 2: kernel=(1,2), stride=(1,2)           -> Output Width: 4
# Final Flatten: 4 * (K_FACTOR * 2) = 256 bits for GroupSum
P1_OUT = 31
K = CONFIG["1d"]["K_FACTOR"]
FLAT_DIM = 4 * K * 2  # 256

class DroneLGN1D(nn.Module):
    """The 1D Logic Gated Network for Drone Detection."""
    def __init__(self, device_str, k=None):
        ...
        self.features = nn.Sequential(
            ...
            LogicConv2d(in_dim=(1, P1_OUT), device=device_str, channels=k, num_kernels=k * 2,
                        receptive_field_size=(1, 16), tree_depth=5,
    ...
    def forward(self, x):
        # 1-bit Binarization (Pure AI approach)
